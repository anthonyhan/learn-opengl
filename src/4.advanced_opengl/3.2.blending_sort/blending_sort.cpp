...
int main()
{
    ...
    // -------------------------
    Shader shader("3.1.blending_discard.vs", "3.1.blending_discard.fs");
    ...
    float episilon = 0.1f;
    glm::vec3 lastCameraPosition = glm::vec3();
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        float movement = glm::length(camera.Position - lastCameraPosition);
        if (movement > episilon) // camera moved
        {
            // sort by distance desc
            std::sort(transparentPositions.begin(), transparentPositions.end(), [=](const glm::vec3& a, const glm::vec3& b)
                {
        ...
        for(size_t i=0; i< transparentPositions.size(); ++i)
        {
            model = glm::translate(glm::mat4(1.0f), transparentPositions[i]);
            shader.setMat4("model",model);
            glDrawArrays(GL_TRIANGLES, 0, 6);
        }       
        glBindVertexArray(0);
