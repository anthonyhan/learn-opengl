...
static const std::vector<std::string> CUBE_MAP_FACES2
{
    "posx.jpg",
    "negx.jpg",
    "posy.jpg",
...
unsigned int loadCubeMap(const char* path, const std::vector<std::string>& faces = CUBE_MAP_FACES);
...
int main()
{
    ...
    // -------------------------
    Shader shader("6.3.objects.vs", "6.3.objects.fs");
    Shader skyboxShader("6.3.skybox.vs", "6.3.skybox.fs");
   ...
   float cubeVertices[] = {
    ...
    // load textures
    // -------------
    unsigned int cubeTexture  = loadTexture(FileSystem::getPath("res/textures/container.jpg").c_str());
    unsigned int floorTexture = loadTexture(FileSystem::getPath("res/textures/metal.png").c_str());
    
    unsigned int cubemapTexture = loadCubeMap(FileSystem::getPath("res/textures/skybox").c_str(), CUBE_MAP_FACES);
...
unsigned int loadCubeMap(const char* path, const std::vector<std::string>& faces)
{
    ...
    for (unsigned int i = 0; i < faces.size(); ++i)
    {
        ...
        if (fullpath[fullpath.size() - 1] != '/' || fullpath[fullpath.size() - 1] != '\\')
            fullpath.append("/");
        ...
        {
            std::cout << "Cubemap texture failed to load at path: " << fullpath.c_str() << std::endl;
            stbi_image_free(data);
        }
    }
