...
int main()
{
	...
	// -----------------------------------------
	// vertex shader
	Shader shader("6.6.shader.vs", "6.6.shader.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (size_t i = 0; i < count; ++i)
		{
			...
			float angle = glm::radians(20.0f * i + 10.0f); //make the first one rotate
			if (i % param.active_index == 0)
...
void imgui_on_render(ui_params& param)
{
	...
	ImGui::SliderInt("Active Index", &param.active_index, 1, 10);
